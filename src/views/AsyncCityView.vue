        ...
        <div class="flex gap-10 overflow-x-scroll">
          ...
          >
            <p class="whitespace-nowrap text-md">
              {{
                new Date(hourData.curentTime).toLocaleTimeString("en-us", {
                  hour: "numeric",
                })
              }}
            </p>
...
const getWeatherData = async () => {
  try {
    const weatherData = await axios.get(
      `https://api.openweathermap.org/data/3.0/onecall?lat=${route.query.lat}&lon=${route.query.lng}&exclude={part}&appid=a1d92ab7fb9873065bac27b5019298f0&units=imperial`
    );
    ...
    weatherData.data.hourly.forEach((hour) => {
      ...
      hour.curentTime = utc + 1000 * weatherData.data.timezone_offset;
    });
